...
- (void)showPasscodeInViewController:(UIViewController *)viewController completion:(PasscodeCompletionBlock)completion {
    ...
    if ([context canEvaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics error:nil]) {
        [context evaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics localizedReason:_config.touchIDReasonString reply:^(BOOL success, NSError* error) {
...
- (void)openPasscodeWithMode:(int)mode viewController:(UIViewController *)viewController {
    ...
    if (_mode == 0) {
        [_passcodeViewController setInstructions:_config.enterNewCodeString];
    } else if (_mode == 1) {
        [_passcodeViewController setInstructions:_config.enterCodeToUnlockString];
    }
}
...
- (void)enteredCode:(NSString *)code {
    if (_mode == 0) {
        if (_count == 0) {
            _prevCode = code;
            [_passcodeViewController setInstructions:_config.repeatCodeString];
            ...
            if ([code isEqualToString:_prevCode]) {
                ...
                [_passcodeViewController setInstructions:_config.enterNewCodeString];
                [_passcodeViewController setErrorMessage:_config.dontMatchString];
                [_passcodeViewController reset];
                _count = 0;
                return;
            }
        ...
        if ([code isEqualToString:[[DMKeychain defaultKeychain] objectForKey:KEYCHAIN_NAME]]) {
            ...
            if (_count == 1) {
                [_passcodeViewController setErrorMessage:_config.oneAttemptLeftString];
            } else {
                [_passcodeViewController setErrorMessage:[NSString stringWithFormat:_config.nAttempsLeftString, 2 - _count]];
            }
            [_passcodeViewController reset];
